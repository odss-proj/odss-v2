...
export default function OwnCloudView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const normalized = allRows.map((r) => {
        ...
        const h3Raw = Number(r["pct_kel_h3"] ?? r["% kel h+3"] ?? r["kel_h3"] ?? 0)
        const h7Raw = Number(r["pct_kel_h7"] ?? r["% kel h+7"] ?? r["kel_h7"] ?? 0)
  ...
  return (
    <div className="space-y-5">
      ...
      {/* TABEL DATA LENGKAP */}
      <OCDataTable data={filtered} />
    </div>
  )
}

function OCDataTable({ data }: { data: OCRow[] }) {
