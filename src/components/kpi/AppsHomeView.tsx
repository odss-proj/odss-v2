...
export default function AppsHomeView() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      const ocData = ocRows.map((r) => {
        // Debug kolom pertama untuk cek nama asli
        if (ocRows.indexOf(r) === 0) {
          console.log("OC raw keys:", Object.keys(r))
          console.log("OC sample row:", r)
        }
        // Coba semua kemungkinan nama kolom
        const h3Raw = Number(
          r["pct_kel_h3"] ?? r["% kel h+3"] ?? r["kel_h3"] ?? r["pct_kelh3"] ??
          r["% KEL H+3"] ?? r["pct kel h3"] ?? r["kel h3"] ?? r["h3"] ?? 0
        )
        const h7Raw = Number(
          r["pct_kel_h7"] ?? r["% kel h+7"] ?? r["kel_h7"] ?? r["pct_kelh7"] ??
          r["% KEL H+7"] ?? r["pct kel h7"] ?? r["kel h7"] ?? r["h7"] ?? 0
        )
        return {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-gray-800">Ringkasan KPI Tim APPS</h2>
