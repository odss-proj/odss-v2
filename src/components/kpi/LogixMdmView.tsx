...
// Tim MDM — filter by br_pic (bukan tas_pic)
const MDM_TEAM = ["IRHANDY", "KUNCORO", "LIA", "WIRA", "MISEL", "MAULANA"]
...
export default function LogixMdmView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const normalized = allRows.map((r) => ({
        ...
        br_pic: String(r["br_pic"] ?? "").toUpperCase().trim(),
      ...
      const appsOnly = normalized.filter((r) => MDM_TEAM.includes(r.br_pic))
      setData(appsOnly)
      setLoading(false)
    }
    fetchData()
  ...
  const filtered = data.filter((r) => {
    if (filterTas !== "ALL" && r.br_pic !== filterTas) return false
  ...
  // By TAS stats
  const tasStats: TasStat[] = MDM_TEAM.map((tas) => {
    const rows = filtered.filter((r) => r.br_pic === tas)
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 flex-wrap bg-white p-4 rounded-2xl border">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {MDM_TEAM.filter((t) => data.some((r) => r.br_pic === t)).map((t) => (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gradient-to-r from-orange-500 to-amber-500 text-white text-xs">
                  ...
                  <th className="px-3 py-3 text-left">BR PIC (MDM)</th>
              ...
              <tbody>
                {paginated.map((r, i) => (
                  <tr key={r.id || i} className={`border-t ${i % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-orange-50`}>
                    ...
                    <td className="px-3 py-2 text-xs font-medium text-gray-700">{r.br_pic}</td>
